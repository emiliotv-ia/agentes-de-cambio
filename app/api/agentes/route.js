  ...
  try {
    ...
    try {
      await resend.emails.send({
        ...
        subject: '🤝 Nuevo Agente de Cambio registrado',
        html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          <div style="background:#0D4F3C;padding:24px;border-radius:12px 12px 0 0">
            <h1 style="color:white;margin:0;font-size:22px">🤝 Nuevo Agente de Cambio</h1>
          </div>
          <div style="background:#F8FAF9;padding:24px;border:1px solid #E5E7EB;border-radius:0 0 12px 12px">
            ...
            <p><strong>Localidad:</strong> ${localidad || 'No indicada'}</p>
            ${motivacion ? `<p><strong>Motivación:</strong> ${motivacion}</p>` : ''}
      ...
      await resend.emails.send({
        ...
        html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          ...
          <div style="background:white;padding:28px 24px;border:1px solid #E5E7EB;border-radius:0 0 12px 12px">
            <p style="color:#374151;font-size:15px;line-height:1.7">Gracias por querer sumarte. Tu intención de ayudar ya es un gran paso.</p>
